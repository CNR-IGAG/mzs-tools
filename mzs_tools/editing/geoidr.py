...
def geoidr_form_init(dialog, layer, feature):
    ...
    alert_text.hide()
    tipo_gi.currentIndexChanged.connect(partial(update_tipo_gi, tipo_gi, valore, valore2, alert_text))
    valore.textEdited.connect(partial(update_valore, valore, tipo_gi, alert_text))
    valore2.textEdited.connect(partial(update_valore2, valore2, tipo_gi))
    help_button.clicked.connect(partial(webbrowser.open, "https://www.youtube.com/watch?v=dnJIjTNzQJQ&t=115s"))
...
def update_valore(value, tipo_gi, alert_text):
    try:
        ...
        if curIndex == "11":
            alert_text.show()
            if valore not in list(range(0, 360)):
                value.setText("")
        if curIndex == "22":
            alert_text.hide()
            if valore == 0:
...
def update_valore2(value, tipo_gi):
    try:
        ...
        curIndex = str(tipo_gi.currentText().strip()).split(" - ")[0]
        if curIndex == "11":
            if valore not in list(range(0, 90)):
                value.setText("")
    except:
        pass
