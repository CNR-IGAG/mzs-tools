...
#		 self.steps = steps
	...
	def work(self):
		...
		# step 1 (presence of project layers)
		###############################################
		self.set_message.emit('Presence of project layers...')
		...
		self.set_log_message.emit("1) Presence of project layers:\n")

		added = self.checkLayers(root, dict_layer, DIZIO_LYR)
		if len(added) == 0:
			self.set_log_message.emit("   The project layers are all present!")
		else:
			self.set_log_message.emit("   The following layers are missing:\n")
			for x in added:
				self.set_log_message.emit("	- " + x + "\n")
		...
		# step 2 (geometric control)
		###############################################
		self.set_message.emit('Geometric control...')
		self.set_log_message.emit("\n\n2) Geometric control:\n")

		for nome in LISTA_LAYER:
			if nome in ["Indagini puntuali", "Parametri puntuali", "Curve di riferimento", "Indagini lineari", "Parametri lineari"]:
				pass
			else:
				self.set_log_message.emit("   I am performing geometric validation of the '" + nome + "' layer\n")
				...
				for feature in features.getFeatures():
					...
					if geom:
						...
						if err:
							self.set_log_message.emit('	%d identified geometric error (feature %d)\n' % (len(err), feature.id()))
				self.set_log_message.emit("   I finished the analysis of the layer '" + nome + "'\n\n")
		...
		# step 3 (topological control)
		###############################################
		self.set_message.emit('Topological control...')
		self.set_log_message.emit("3) Topological control:\n")

		self.set_log_message.emit("   I am performing topological validation of 'Carta Geotecnica' level...\n")
		if intersezioni_geotec is True:
			...
			self.set_log_message.emit("	Done! File containing auto-intersections of 'Unita' geologico-tecniche' layer has been saved in '\\allegati\\log\\analisi\\geotec_self_inters.shp'\n\n")
		else:
			self.set_log_message.emit("	Topological validation can not be performed because one or more layers is/are missing!\n\n")

		self.set_log_message.emit("   I am performing topological validation of the 'MS1' level...\n")
		if intersezioni_ms1 is True:
			self.topology_check(pathname + os.sep + "analisi", "Zone stabili liv 1", "Zone instabili liv 1", "ID_z", "ID_i", "stab_1_self_inters", "instab_1_self_inters", "ms1_inters_stab_instab")
		else:
			self.set_log_message.emit("	Topological validation can not be performed because one or more layers is/are missing!\n\n")

		self.set_log_message.emit("   I am performing topological validation of the 'MS2' level...\n")
		if intersezioni_ms2 is True:
			self.topology_check(pathname + os.sep + "analisi", "Zone stabili liv 2", "Zone instabili liv 2", "ID_z", "ID_i", "stab_2_self_inters", "instab_2_self_inters", "ms2_inters_stab_instab")
		else:
			self.set_log_message.emit("	Topological validation can not be performed because one or more layers is/are missing!\n\n")

		self.set_log_message.emit("   I am performing topological validation of the 'MS3' level...\n")
		if intersezioni_ms3 is True:
			self.topology_check(pathname + os.sep + "analisi", "Zone stabili liv 3", "Zone instabili liv 3", "ID_z", "ID_i", "stab_3_self_inters", "instab_3_self_inters", "ms3_inters_stab_instab")
		else:
			self.set_log_message.emit("	Topological validation can not be performed because one or more layers is/are missing!\n\n")
...
##		root = QgsProject.instance().layerTreeRoot()
	...
	def topology_check(self, directory, lyr1, lyr2, campo1, campo2, nome1, nome2, nome3):
...
##		self.set_log_message.emit("	Done! File containing auto-intersections of '" + lyr1 + "' layer has been saved in '\\allegati\\log\\analisi\\" + nome1 + ".shp'\n")
...
##		self.set_log_message.emit("	Done! File containing auto-intersections of '" + lyr2 + "' layer has been saved in '\\allegati\\log\\analisi\\" + nome2 + ".shp'\n")
...
##		self.set_log_message.emit("	Done! File containing intersections between '" + lyr1 + "' and '" + lyr2 + "' layers was saved in '\\allegati\\log\\analisi\\" + nome3 + ".shp'\n\n")
