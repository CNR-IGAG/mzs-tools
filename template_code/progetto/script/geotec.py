...
def geotec(dialog, layer, feature):
    ...
    tipo_gt.currentIndexChanged.connect(
        partial(update_box_stato, tipo_gt, stato_box, codici_stato))
    stato_box.currentIndexChanged.connect(
        partial(update_stato, stato, stato_box))
    define_gen(codici_gen)
    tipo_gt.currentIndexChanged.connect(
        partial(update_box_gen, tipo_gt, gen_box, codici_gen))
    gen_box.currentIndexChanged.connect(partial(update_gen, gen, gen_box))
    tipo_gt.currentIndexChanged.connect(
        partial(update_alert, tipo_gt, gen_box, stato_box, alert_text))
    help_button.clicked.connect(lambda: webbrowser.open(
        'https://www.youtube.com/watch?v=dnJIjTNzQJQ&t=115s'))
...
def define_stato(codici_stato):
    ...
    for classe in codici_stato_layer.getFeatures(QgsFeatureRequest().setFlags(QgsFeatureRequest.NoGeometry)):
        lista_stato = [classe.attributes()[1], classe.attributes()[
...
def define_gen(codici_gen):
    ...
    for classe in codici_gen_layer.getFeatures(QgsFeatureRequest().setFlags(QgsFeatureRequest.NoGeometry)):
        lista_gen = [classe.attributes()[1], classe.attributes()[
            2], classe.attributes()[3]]
...
def update_alert(tipo_gt, gen_box, stato_box, alert_text):
    ...
    if curIndex in ("RI", "GW", "GP", "GM", "GC", "SW", "SP", "SM", "SC", "OL", "OH", "MH", "ML", "CL", "CH", "PT", "LC"):
        alert_text.hide()
        gen_box.setEnabled(True)
        stato_box.setEnabled(True)
    elif curIndex in ("LP", "GR", "CO", "AL", "LPS", "GRS", "COS", "ALS", "SFLP", "SFGR", "SFCO", "SFAL", "SFLPS", "SFGRS", "SFCOS", "SFALS", "IS", "ISS", "SFIS", "SFISS"):
        alert_text.show()
