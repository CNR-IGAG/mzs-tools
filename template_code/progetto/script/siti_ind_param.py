...
def indagini_puntuali(dialog, layer, feature):
    ...
    id_spu.currentIndexChanged.connect(
        partial(update_pkey, id_spu, pkey_spu, 'P'))
    tab.currentChanged.connect(
        partial(refresh_variable_ind, classe_ind, doc_ind))
    ...
    tipo_ind_box.currentIndexChanged.connect(
        partial(update_tipo_ind_p, tipo_ind, tipo_ind_box))
    button_doc.clicked.connect(
        partial(select_output_file, button_doc, doc_ind))
    tipo_ind_box.currentIndexChanged.connect(
        partial(disableButton_p, tipo_ind_box, classe_ind, id_spu, buttonBox))
    classe_ind.currentIndexChanged.connect(
        partial(disableButton_p, tipo_ind_box, classe_ind, id_spu, buttonBox))
    id_spu.currentIndexChanged.connect(partial(disableButton_p,
                                               tipo_ind_box, classe_ind, id_spu, buttonBox))
    ...
    prof_bot.editingFinished.connect(
        partial(alert_spessore, prof_top, prof_bot, 0))
    prof_top.editingFinished.connect(
        partial(alert_spessore, prof_top, prof_bot, 0))
    quota_slm_top.editingFinished.connect(
        partial(alert_spessore, quota_slm_bot, quota_slm_top, 1))
    quota_slm_bot.editingFinished.connect(
        partial(alert_spessore, quota_slm_bot, quota_slm_top, 1))
    id_indpu.textChanged.connect(partial(update_name_combobox,
                                         layer, tipo_ind_box, tipo_ind, codici_indagini))
    help_button.clicked.connect(lambda: webbrowser.open(
        'https://www.youtube.com/watch?v=aP2buSJk8iE&t=5s'))
...
def indagini_lineari(dialog, layer, feature):
    ...
    id_sln.currentIndexChanged.connect(
        partial(update_pkey, id_sln, pkey_sln, 'L'))
    tab.currentChanged.connect(
        partial(refresh_variable_ind, classe_ind, doc_ind))
    classe_ind.currentIndexChanged.connect(partial(update_box_ind,
                                                   classe_ind, tipo_ind_box, codici_indagini))
    ...
    id_indln.textChanged.connect(partial(update_name_combobox,
                                         layer, tipo_ind_box, tipo_ind, codici_indagini))
    help_button.clicked.connect(lambda: webbrowser.open(
        'https://www.youtube.com/watch?v=aP2buSJk8iE&t=5s'))
...
def parametri_puntuali(dialog, layer, feature):
    ...
    if id_indpu.currentText() != '':
        id_indpu.currentIndexChanged.connect(partial(update_param_p,
                                                     codici_parametri, id_indpu, tipo_parpu_box))
    ...
    id_indpu.currentIndexChanged.connect(
        partial(update_pkey_par, id_indpu, pkey_indpu, 'P'))
    tab.currentChanged.connect(partial(refresh_variable_par_p,
                                       valore_box, valore, tipo_parpu_box, tipo_parpu, codici_parametri, id_indpu))
    tipo_parpu_box.currentIndexChanged.connect(
        partial(update_tipo_par, tipo_parpu, tipo_parpu_box))
    tipo_parpu_box.currentIndexChanged.connect(
        partial(disableButton, tipo_parpu_box, buttonBox))
    ...
    if layer.isEditable():
        tipo_parpu_box.currentIndexChanged.connect(partial(change_valore_p,
                                                           tipo_parpu_box, valore, valore_box, simbolo_val, codici_parametri))
        valore_box.currentIndexChanged.connect(
            partial(update_valore_text, valore, valore_box))
    else:
        tipo_parpu_box.currentIndexChanged.connect(partial(not_edit_change_p,
                                                           tipo_parpu_box, tipo_parpu, valore_box, valore, simbolo_val, codici_parametri))

    tipo_parpu_box.currentIndexChanged.connect(partial(update_name_combobox,
                                                       layer, tipo_parpu_box, tipo_parpu, codici_parametri))
    help_button.clicked.connect(lambda: webbrowser.open(
        'https://www.youtube.com/watch?v=-tjezNh1m1A&t=1s'))
...
def parametri_lineari(dialog, layer, feature):
    ...
    if id_indln.currentText() != '':
        id_indln.currentIndexChanged.connect(partial(update_param_l, codici_parametri, id_indln, tipo_parln_box, attend_mis,
                                                     note_par, prof_bot, prof_top, quota_slm_bot, quota_slm_top, valore, data_par, alert_text, buttonBox))
        update_param_l(codici_parametri, id_indln, tipo_parln_box, attend_mis, note_par, prof_bot,
                       prof_top, quota_slm_bot, quota_slm_top, valore, data_par, alert_text, buttonBox)
    else:
        id_indln.currentIndexChanged.connect(partial(update_param_l, codici_parametri, id_indln, tipo_parln_box, attend_mis,
                                                     note_par, prof_bot, prof_top, quota_slm_bot, quota_slm_top, valore, data_par, alert_text, buttonBox))
    ...
    id_indln.currentIndexChanged.connect(
        partial(update_pkey_par, id_indln, pkey_indln, 'L'))
    id_indln.currentIndexChanged.connect(partial(refresh_variable_par_l,
                                                 valore, tipo_parln_box, tipo_parln, attend_mis, codici_parametri, id_indln))
    tipo_parln_box.currentIndexChanged.connect(
        partial(update_tipo_par, tipo_parln, tipo_parln_box))
    tipo_parln_box.currentIndexChanged.connect(
        partial(disableButton, tipo_parln_box, buttonBox))
    prof_top.textEdited.connect(partial(update_valore, prof_top))
    prof_bot.textEdited.connect(partial(update_valore, prof_bot))
    quota_slm_top.textEdited.connect(partial(update_valore_slm, quota_slm_top))
    quota_slm_bot.textEdited.connect(partial(update_valore_slm, quota_slm_bot))
    ...
    tipo_parln_box.currentIndexChanged.connect(partial(update_name_combobox,
                                                       layer, tipo_parln_box, tipo_parln, codici_parametri))
    help_button.clicked.connect(lambda: webbrowser.open(
        'https://www.youtube.com/watch?v=-tjezNh1m1A&t=1s'))
...
def curve(dialog, layer, feature):
    ...
    if id_parpu.currentText() != '':
        id_parpu.currentIndexChanged.connect(
            partial(disableButton_curve, id_parpu, buttonBox))
    ...
    id_parpu.currentIndexChanged.connect(
        partial(update_pkey_par, id_parpu, pkey_parpu, 'C'))

    help_button.clicked.connect(lambda: webbrowser.open(
        'https://www.youtube.com/watch?v=EPRPjKvbUwE'))
...
def freq(dialog, layer, feature):
    ...
    buttonBox.setEnabled(False)
    id_indpu.currentIndexChanged.connect(
        partial(refresh_variable_freq, qualita, tipo))
    ...
    help_button.clicked.connect(lambda: webbrowser.open(
        'https://www.youtube.com/watch?v=PDjnJThqHE8'))
...
def update_pkey_par(value1, value2, param):
    ...
    if param == 'P':
        lista_var = ['CD', 'CU', 'UU', 'ELL', 'CR', 'BE', 'TSC', 'TTC', 'TC', 'CPT', 'CPTE', 'CPTU', 'SPT', 'DMT', 'VT', 'DS', 'DP', 'DN', 'DL', 'PP', 'PLT', 'AL', 'AIV', 'S', 'SS', 'SD',
                     'SDS', 'SC', 'SP', 'SI', 'PI', 'T', 'RGM', 'TP', 'GEO', 'PA', 'SP', 'LF', 'ST', 'PE', 'ERT', 'SDMT', 'SCPT', 'DH', 'CH', 'UH', 'HVSR', 'ACC', 'ESAC_SPAC', 'GM', 'AR', 'SL']
        for x in lista_var:
            if x in value1.currentText():
                value2.setText(value1.currentText().split(x)[-1])
    elif param == 'L':
        lista_var = ['STP', 'SGE', 'SEV', 'SEO', 'PR', 'ERT',
                     'SL', 'SR', 'RAD', 'MASW', 'SASW', 'REMI', 'FTAN']
        for x in lista_var:
            if x in value1.currentText():
                value2.setText(value1.currentText().split(x)[-1])
    elif param == 'C':
        lista_var = ['PV', 'E1', 'DR', 'W', 'IP', 'GH', 'SA', 'LM', 'AR', 'OC', 'C', 'F1', 'CU', 'G', 'RT', 'IS', 'II', 'DV', 'E', 'CP', 'QC', 'FS', 'U', 'PT', 'KR', 'PTS', 'SPT', 'PTM',
                     'PTL', 'SIG', 'PIA', 'IL', 'FDS', 'KC', 'KEQ', 'L', 'SG', 'CAM', 'INC', 'PS', 'JV', 'FAG', 'FRA', 'LID', 'FF', 'FP', 'K', 'T', 'RHO', 'VS', 'VP', 'FR', 'ACS', 'ACB', 'ACI', 'ACO']
        for x in lista_var:
            if x in value1.currentText():
                value2.setText(value1.currentText().split(x)[-1])
...
def select_output_file(button_doc, doc_ind):
    ...
    filedirectory, __ = QFileDialog.getOpenFileName(
        button_doc, "Select output file ", "", '*.pdf')
...
def define_tipo_ind(nome_tab):
    ...
    for classe in codici_indagini_layer.getFeatures(QgsFeatureRequest().setFlags(QgsFeatureRequest.NoGeometry)):
        lista_indagini = [classe.attributes()[1], classe.attributes()[
            2], classe.attributes()[3]]
...
def disableButton_p(input1, input2, input3, buttonBox):
    ...
    check_campi = [input1.currentText(), input2.currentText(),
                   input3.currentText()]
    check_value = []
...
def define_param(codici_parametri, nome_tabella):
    ...
    codici_parametri_layer = QgsProject.instance().mapLayersByName(nome_tabella)[
        0]
    for param in codici_parametri_layer.getFeatures(QgsFeatureRequest().setFlags(QgsFeatureRequest.NoGeometry)):
        lista_parametri = [param.attributes()[1], param.attributes()[
            2], param.attributes()[3], param.attributes()[4]]
...
def define_valore(codici_valori, valore_box):
    ...
    valore_box.clear()
    valore_box.addItem("")
    codici_valori_layer = QgsProject.instance(
    ).mapLayersByName("vw_tipo_gt")[0]
